...
- ✅ 阶段一：高严重度 Bug 1–5 全部修复
- ✅ 阶段二：中严重度 Bug 6–15 全部修复
- ✅ 阶段三：死代码清理、配置常量收敛、populate 按需化、包重命名、
  通用后台列表组件（新集合已使用；旧 14 个列表页可后续逐步迁移）
- ✅ 阶段四：school 集合 + 幂等种子（已在全新数据库上实测三语种子成功）+
  学生 school_ref 回填 + 前端全面改读后台数据（旧枚举仅作回退）
- ✅ 阶段五：反向链接（结构化 + wiki 链接）、考据对象枢纽页、语义化关系、
  剧透分级（阅读进度 + 卡片模糊 + 详情剧透门）、多阅读路径（列表/详情/
  条目内上一篇下一篇）、修订记录时间线、wiki 式 [[链接]]（含悬浮摘要）、
  知识图谱（无依赖 Canvas 力导向，可拖拽/缩放/点击跳转）
- ✅ 阶段六：前后端 tsc/ESLint 零错误；`next build` 通过；
  全新 Strapi 实测启动 + 公开 API 验证；`next start` 冒烟全部 200
- ⚠️ e2e：用例已补充（考据档案、对象、路径、图谱、公告、新后台路由保护），
  但本环境网络策略禁止下载 Playwright 浏览器，需在 CI 或本地执行
  `pnpm test:e2e` 验证

### 留待后续的低优先项

- panel.ts 进一步拆分（已抽出 job-lock 工具；质量扫描/CSV 导出仍在控制器内）
- 旧 14 个 manage 列表页迁移到 `AdminGenericListPage`
- API 输入引入 zod schema 校验（当前为白名单字段 + 类型归一化）
- 旧 `student.school` 枚举与 `affiliations` JSON 的最终下线（待数据全部迁移后）

### 部署提示

- Strapi 启动时会自动迁移数据库结构并幂等播种 13 所学院、回填学生学院关联，
  无需手工脚本；重复启动不会覆盖编辑过的数据。

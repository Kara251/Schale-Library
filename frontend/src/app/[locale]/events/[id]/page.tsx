import { notFound } from 'next/navigation'
import Image from 'next/image'
import { Header } from "@/components/header"
import { Footer } from "@/components/footer"
import { Button } from "@/components/ui/button"
import { Badge } from "@/components/ui/badge"
import { EventCard } from "@/components/event-card"
import { Calendar, CheckCircle, ExternalLink, Globe, MapPin, Ticket, User, ArrowLeft } from 'lucide-react'
import { getEventById, getOfflineEvents, getOnlineEvents, type OfflineEvent } from "@/lib/api"
import {
    formatEventPrice,
    getEventDisplayPlace,
    getEventSourceHost,
    getEventStatusOverrideLabel,
    splitEventTags,
} from "@/lib/utils/event-display"
import { sanitizeHtml } from "@/lib/sanitize"
import { format } from 'date-fns'
import { zhCN, enUS, ja } from 'date-fns/locale'
import { LocaleLink } from '@/components/locale-link'
import type { Locale } from '@/lib/i18n'
import { getMediaUrl } from '@/lib/media'
import { SITE_URL } from '@/lib/config'

export const revalidate = 60;

interface PageProps {
    params: Promise<{ id: string; locale: string }>
}

const dateLocales = { 'zh-Hans': zhCN, 'en': enUS, 'ja': ja }

const content: Record<Locale, {
    back: string
    kindOnline: string
    kindOffline: string
    upcoming: string
    ongoing: string
    ended: string
    official: string
    fanmade: string
    organizer: string
    eventPeriod: string
    timeNote: string
    startsIn: string
    endsIn: string
    endedAgo: string
    source: string
    lastVerifiedAt: string
    address: string
    ticket: string
    ticketLink: string
    mapLink: string
    verified: string
    tags: string
    location: string
    visitLink: string
    description: string
    relatedOnline: string
    relatedOffline: string
}> = {
    'zh-Hans': {
        back: '返回活动列表',
        kindOnline: '线上',
        kindOffline: '线下',
        upcoming: '未开始',
        ongoing: '进行中',
        ended: '已结束',
        official: '官方',
        fanmade: '同人',
        organizer: '主办方',
        eventPeriod: '活动时间',
        timeNote: '时间提示',
        startsIn: '距离开始约 {days} 天',
        endsIn: '距离结束约 {days} 天',
        endedAgo: '已结束约 {days} 天',
        source: '来源',
        lastVerifiedAt: '最后核验',
        address: '地址',
        ticket: '票务',
        ticketLink: '票务 / 报名',
        mapLink: '地图',
        verified: '已认证',
        tags: '标签',
        location: '活动地点',
        visitLink: '访问活动',
        description: '活动详情',
        relatedOnline: '相关线上活动',
        relatedOffline: '相关线下活动',
    },
    'en': {
        back: 'Back to events',
        kindOnline: 'Online',
        kindOffline: 'Offline',
        upcoming: 'Upcoming',
        ongoing: 'Ongoing',
        ended: 'Ended',
        official: 'Official',
        fanmade: 'Fan-made',
        organizer: 'Organizer',
        eventPeriod: 'Event Period',
        timeNote: 'Timing',
        startsIn: 'Starts in about {days} days',
        endsIn: 'Ends in about {days} days',
        endedAgo: 'Ended about {days} days ago',
        source: 'Source',
        lastVerifiedAt: 'Last verified',
        address: 'Address',
        ticket: 'Ticket',
        ticketLink: 'Ticket / registration',
        mapLink: 'Map',
        verified: 'Verified',
        tags: 'Tags',
        location: 'Location',
        visitLink: 'Visit Event',
        description: 'Details',
        relatedOnline: 'Related Online Events',
        relatedOffline: 'Related Offline Events',
    },
    'ja': {
        back: 'イベント一覧に戻る',
        kindOnline: 'オンライン',
        kindOffline: 'オフライン',
        upcoming: '未開始',
        ongoing: '開催中',
        ended: '終了',
        official: '公式',
        fanmade: '二次創作',
        organizer: '主催者',
        eventPeriod: '開催期間',
        timeNote: '時間メモ',
        startsIn: '開始まで約{days}日',
        endsIn: '終了まで約{days}日',
        endedAgo: '終了から約{days}日',
        source: '出典',
        lastVerifiedAt: '最終確認',
        address: '住所',
        ticket: 'チケット',
        ticketLink: 'チケット / 申込',
        mapLink: '地図',
        verified: '確認済み',
        tags: 'タグ',
        location: '開催場所',
        visitLink: 'イベントを見る',
        description: '詳細',
        relatedOnline: '関連オンラインイベント',
        relatedOffline: '関連オフラインイベント',
    },
}

export async function generateMetadata({ params }: PageProps) {
    const { id, locale } = await params
    const found = await getEventById(id, locale).catch(() => null)

    if (!found) {
        return { title: 'Event not found - Schale Library' }
    }

    const event = found.event
    const siteUrl = SITE_URL
    const imageUrl = event.coverImage?.url ? new URL(getMediaUrl(event.coverImage.url), siteUrl).toString() : undefined
    const description = event.description?.replace(/<[^>]*>/g, '').slice(0, 150) || ''

    return {
        title: `${event.title} - Schale Library`,
        description,
        openGraph: {
            title: `${event.title} - Schale Library`,
            description,
            type: 'article',
            images: imageUrl ? [{ url: imageUrl }] : undefined,
        },
    }
}

function getSourceHost(link?: string) {
    if (!link) return null
    try {
        return new URL(link).hostname.replace(/^www\./, '')
    } catch {
        return null
    }
}

export default async function EventDetailPage({ params }: PageProps) {
    const { id, locale } = await params
    const t = content[locale as Locale] || content['zh-Hans']
    const dateLocale = dateLocales[locale as Locale] || zhCN

    const found = await getEventById(id, locale).catch(() => null)

    if (!found || !found.event) {
        notFound()
    }

    const { event, kind } = found
    const isOffline = kind === 'offline'

    const relatedRes = await (kind === 'online' ? getOnlineEvents : getOfflineEvents)(4, locale, {
        nature: event.nature,
        sort: 'relevant',
        excludeId: event.id,
        pageSize: 4,
    }).catch((error) => {
        console.error('Failed to load related events:', error)
        return { data: [] }
    })
    const relatedEvents = relatedRes.data || []

    const formatDate = (dateString: string) => {
        try {
            return format(new Date(dateString), 'yyyy-MM-dd HH:mm', { locale: dateLocale })
        } catch {
            return dateString
        }
    }

    const getEventStatus = () => {
        const overrideLabel = getEventStatusOverrideLabel(event.statusOverride, locale as Locale)
        if (overrideLabel) return { label: overrideLabel, color: 'default' as const }

        const now = new Date()
        const start = new Date(event.startTime)
        const end = new Date(event.endTime)

        if (now < start) return { label: t.upcoming, color: 'default' as const }
        if (now > end) return { label: t.ended, color: 'secondary' as const }
        return { label: t.ongoing, color: 'default' as const }
    }

    const status = getEventStatus()
    const natureLabel = event.nature === 'official' ? t.official : t.fanmade
    const kindLabel = isOffline ? t.kindOffline : t.kindOnline
    const formatDayDistance = () => {
        const now = new Date()
        const start = new Date(event.startTime)
        const end = new Date(event.endTime)
        const dayMs = 24 * 60 * 60 * 1000

        if (now < start) return t.startsIn.replace('{days}', String(Math.max(1, Math.ceil((start.getTime() - now.getTime()) / dayMs))))
        if (now <= end) return t.endsIn.replace('{days}', String(Math.max(1, Math.ceil((end.getTime() - now.getTime()) / dayMs))))
        return t.endedAgo.replace('{days}', String(Math.max(1, Math.ceil((now.getTime() - end.getTime()) / dayMs))))
    }
    const sourceHost = getSourceHost(event.link)
    const displayPlace = getEventDisplayPlace(event, kind, locale as Locale)
    const priceLabel = formatEventPrice(event, locale as Locale)
    const verifiedAt = event.lastVerifiedAt ? formatDate(event.lastVerifiedAt) : ''
    const tags = splitEventTags(event.tags)
    const sourceDisplayHost = getEventSourceHost(event) || sourceHost
    const sourceLabel = [event.sourceName, sourceDisplayHost].filter(Boolean).join(' / ')
    const offlineEvent = isOffline ? (event as OfflineEvent) : null

    return (
        <div className="min-h-screen flex flex-col">
            <Header />

            <main className="relative flex-1 container mx-auto px-4 pt-6 pb-12">
                <div className="content-panel">
                    <LocaleLink href="/events" className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground mb-6">
                        <ArrowLeft className="h-4 w-4" />
                        {t.back}
                    </LocaleLink>

                    <div className="max-w-4xl mx-auto">
                        {event.coverImage && (
                            <div className="relative h-64 md:h-96 rounded-lg overflow-hidden mb-8">
                                <Image src={getMediaUrl(event.coverImage.url)} alt={event.title} fill priority sizes="(max-width: 768px) 100vw, 896px" className="object-cover" />
                                <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
                                <div className="absolute top-4 left-4 flex gap-2">
                                    <Badge variant={status.color} className="font-bold">{status.label}</Badge>
                                    <Badge variant={event.nature === 'official' ? 'default' : 'secondary'} className="font-bold">{natureLabel}</Badge>
                                </div>
                            </div>
                        )}

                        <div className="mb-8">
                            <h1 className="text-3xl md:text-4xl font-bold mb-4">{event.title}</h1>
                            <div className="mb-4 flex flex-wrap gap-2">
                                <Badge variant={status.color} className="font-bold">{status.label}</Badge>
                                <Badge variant={event.nature === 'official' ? 'default' : 'secondary'} className="font-bold">{natureLabel}</Badge>
                                <Badge variant="outline" className="font-bold">{kindLabel}</Badge>
                            </div>

                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                                {event.organizer && (
                                    <div className="flex items-center gap-2">
                                        <User className="h-4 w-4 text-muted-foreground" />
                                        <span className="text-muted-foreground">{t.organizer}:</span>
                                        <span>{event.organizer}</span>
                                        {event.organizerVerified ? (
                                            <span className="inline-flex items-center gap-1 text-primary">
                                                <CheckCircle className="h-3.5 w-3.5" />
                                                {t.verified}
                                            </span>
                                        ) : null}
                                    </div>
                                )}
                                <div className="flex items-center gap-2">
                                    <Calendar className="h-4 w-4 text-muted-foreground" />
                                    <span className="text-muted-foreground">{t.eventPeriod}:</span>
                                    <span>{formatDate(event.startTime)} - {formatDate(event.endTime)}</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <Calendar className="h-4 w-4 text-muted-foreground" />
                                    <span className="text-muted-foreground">{t.timeNote}:</span>
                                    <span>{formatDayDistance()}</span>
                                </div>
                                {displayPlace && !isOffline && (
                                    <div className="flex items-center gap-2">
                                        <Globe className="h-4 w-4 text-muted-foreground" />
                                        <span className="text-muted-foreground">{t.location}:</span>
                                        <span>{displayPlace}</span>
                                    </div>
                                )}
                                {displayPlace && isOffline && (
                                    <div className="flex items-center gap-2 md:col-span-2">
                                        <MapPin className="h-4 w-4 text-muted-foreground" />
                                        <span className="text-muted-foreground">{t.location}:</span>
                                        <span>{displayPlace}</span>
                                    </div>
                                )}
                                {offlineEvent?.address ? (
                                    <div className="flex items-center gap-2 md:col-span-2">
                                        <MapPin className="h-4 w-4 text-muted-foreground" />
                                        <span className="text-muted-foreground">{t.address}:</span>
                                        <span>{offlineEvent.address}</span>
                                    </div>
                                ) : null}
                                {priceLabel ? (
                                    <div className="flex items-center gap-2">
                                        <Ticket className="h-4 w-4 text-muted-foreground" />
                                        <span className="text-muted-foreground">{t.ticket}:</span>
                                        <span>{priceLabel}</span>
                                    </div>
                                ) : null}
                                {!isOffline && sourceLabel ? (
                                    <div className="flex items-center gap-2">
                                        <Globe className="h-4 w-4 text-muted-foreground" />
                                        <span className="text-muted-foreground">{t.source}:</span>
                                        <span>{sourceLabel}</span>
                                    </div>
                                ) : null}
                                {isOffline && sourceLabel ? (
                                    <div className="flex items-center gap-2 md:col-span-2">
                                        <ExternalLink className="h-4 w-4 text-muted-foreground" />
                                        <span className="text-muted-foreground">{t.source}:</span>
                                        <span>{sourceLabel}</span>
                                    </div>
                                ) : null}
                                {verifiedAt ? (
                                    <div className="flex items-center gap-2">
                                        <CheckCircle className="h-4 w-4 text-muted-foreground" />
                                        <span className="text-muted-foreground">{t.lastVerifiedAt}:</span>
                                        <span>{verifiedAt}</span>
                                    </div>
                                ) : null}
                            </div>
                        </div>

                        {tags.length > 0 ? (
                            <div className="mb-8 flex flex-wrap gap-2">
                                <span className="text-sm text-muted-foreground">{t.tags}:</span>
                                {tags.map((tag) => (
                                    <Badge key={tag} variant="secondary">{tag}</Badge>
                                ))}
                            </div>
                        ) : null}

                        {(event.ticketUrl || offlineEvent?.mapUrl || event.link) && (
                            <div className="mb-8 flex flex-wrap gap-3">
                                {event.ticketUrl ? (
                                    <Button asChild size="lg" className="w-full md:w-auto">
                                        <a href={event.ticketUrl} target="_blank" rel="noopener noreferrer">
                                            <Ticket className="mr-2 h-4 w-4" />
                                            {t.ticketLink}
                                        </a>
                                    </Button>
                                ) : null}
                                {offlineEvent?.mapUrl ? (
                                    <Button asChild size="lg" variant="outline" className="w-full md:w-auto">
                                        <a href={offlineEvent.mapUrl} target="_blank" rel="noopener noreferrer">
                                            <MapPin className="mr-2 h-4 w-4" />
                                            {t.mapLink}
                                        </a>
                                    </Button>
                                ) : null}
                                {event.link ? (
                                    <Button asChild size="lg" className="w-full md:w-auto">
                                        <a href={event.link} target="_blank" rel="noopener noreferrer">
                                            {isOffline ? (
                                                <ExternalLink className="mr-2 h-4 w-4" />
                                            ) : (
                                                <Globe className="mr-2 h-4 w-4" />
                                            )}
                                            {t.visitLink}
                                        </a>
                                    </Button>
                                ) : null}
                            </div>
                        )}

                        {event.description && (
                            <div className="prose prose-slate dark:prose-invert max-w-none">
                                <h2 className="text-2xl font-bold mb-4">{t.description}</h2>
                                <div className="bg-card border rounded-lg p-6" dangerouslySetInnerHTML={{ __html: sanitizeHtml(event.description) }} />
                            </div>
                        )}

                        {relatedEvents.length > 0 ? (
                            <section className="mt-10">
                                <h2 className="text-2xl font-bold mb-4">{isOffline ? t.relatedOffline : t.relatedOnline}</h2>
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                                    {relatedEvents.map((item) => (
                                        <EventCard key={item.id} event={item} type={kind} />
                                    ))}
                                </div>
                            </section>
                        ) : null}
                    </div>
                </div>
            </main>

            <Footer />
        </div>
    )
}

...
interface ResearchThemeAdminEntry extends AdminStrapiEntry {
  name: string
  slug?: string
}
...
const labels: Record<Locale, {
  ...
  en: {
    title: 'Research Theme Management',
    description: 'Manage theme tags that span multiple entries.',
    search: 'Filter',
    searchPlaceholder: 'Search theme name',
    reset: 'Reset',
    statusAll: 'All statuses',
    statusPublished: 'Published',
    statusDraft: 'Draft',
    empty: 'No themes matched the current filters.',
    previous: 'Previous',
    next: 'Next',
  ...
  ja: {
    title: '考察テーマ管理',
    description: '複数記事を横断するテーマタグを管理します。',
    search: '絞り込み',
    searchPlaceholder: 'テーマ名を検索',
    reset: 'リセット',
    statusAll: 'すべての状態',
    statusPublished: '公開済み',
    statusDraft: '下書き',
    empty: '条件に一致する考察テーマがありません。',
    previous: '前へ',
    next: '次へ',
...
export default async function ResearchThemesManagePage({ params, searchParams }: ResearchThemesManagePageProps) {
  ...
  const session = await requireAdminSession(locale, `/${locale}/manage/research-themes`)
  ...
  const response = await listAdminCollection<ResearchThemeAdminEntry>(session, 'research-themes', {
    locale,
    page,
    search: query.search,
    status,
  ...
  const buildHref = (nextPage: number) => {
    ...
    const qs = p.toString()
    return `/${locale}/manage/research-themes${qs ? `?${qs}` : ''}`
  }
  ...
  return (
    <div>
      <AdminPageHeader
        ...
        actions={
          <Button asChild>
            <Link href={`/${locale}/manage/research-themes/new`}>{actionLabels.create}</Link>
          </Button>
        }
      />
      <AdminSearchForm
        action={`/${locale}/manage/research-themes`}
        search={query.search}
        status={status}
      ...
      <AdminTable
        ...
        columns={[
          ...
          },
          {
            ...
            header: t.slug,
            className: 'w-40',
            render: (item) => <span className="text-xs text-muted-foreground">{item.slug || '-'}</span>,
          },
          {
            key: 'publishedAt',

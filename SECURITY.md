...
The patched versions reported by the audit database are not currently resolvable from npm for this dependency tree (`lodash >=4.18.0`, `vite 6.4.2`, `uuid >=14.0.0`). Do not force these overrides unless Strapi supports them and both frontend/backend builds pass.
...
- Sync logs are read-only through the custom panel API.
